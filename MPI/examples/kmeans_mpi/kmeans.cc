...
int main(int argc, char **argv) {
  ...
  // allocate memory
  if(myrank==0) my_num_points = num_points/num_workers+num_points%num_workers;
  else my_num_points = num_points/num_workers;
  ...
  for(int i=0; i<num_iter; i++) {
    ...
    for(int c=0; c<num_clusters; c++) num_assigned_local[c]=0;
