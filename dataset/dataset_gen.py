...
num_rows = float(sys.argv[2]) # number of rows to generate
decimal_part, integer_part = modf(num_rows)

print "Generating new dataset with %sM rows... " % (sys.argv[2])
...
dataframe_output.to_csv('collisions_' + sys.argv[2] + 'M.csv', index=False)
print "Done."